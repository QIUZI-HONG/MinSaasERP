...
await test('并发 10 个请求抢购库存仅 5 件的商品：恰好成功 5 个，不超卖', async () => {
  ...
  const created = await req(
    'POST',
    '/products',
    { sku, name: '并发测试商品', price: 10, stock: 5 },
    authHeaders()
  );
  ...
  observe(
    `超卖并发结果：成功 ${okCount}，库存不足拒绝 ${failCount}，服务器错误 ${serverErr}${serverErr > 0 ? '（⚠️ 并发下出现 500）' : ''}`
  );
});
...
await test('并发 15 单：订单号无重复', async () => {
  ...
  observe(
    `订单号唯一性：成功 ${created.length} 单全部唯一${failed.length > 0 ? `，失败 ${failed.length} 单（原因见下）` : ''}`
  );
...
await test('下单-取消循环 5 轮后库存精确回补', async () => {
  ...
  for (let i = 0; i < 5; i++) {
    const o = await req(
      'POST',
      '/orders',
      { customerId: 4, items: [{ productId: pid, quantity: 2 }] },
      authHeaders()
    );
...
await test('超长商品名（2000 字符）被拒绝（P3 缺陷修复）', async () => {
  ...
  const r = await req(
    'POST',
    '/products',
    { sku: 'LONG-' + Date.now(), name: longName, price: 1 },
    authHeaders()
  );
...
await test('极端数值：价格 99999999.99 与数量 1000000', async () => {
  ...
  await req(
    'POST',
    '/products',
    { sku, name: '大数值商品', price: 99999999.99, stock: 1000000 },
    authHeaders()
  );
  const products = (await req('GET', `/products?q=${sku}`, undefined, authHeaders())).data;
  const pid = products[0].id;
  const r = await req(
    'POST',
    '/orders',
    { customerId: 4, items: [{ productId: pid, quantity: 1000000 }] },
    authHeaders()
  );
